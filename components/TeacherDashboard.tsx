...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ topics, onAssignMission }) => {
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of fileArray) {
      ...
      let masterySum = 0, count = 0, c1=0, c2=0, c3=0, c4=0;
      ...
      processedStudents.push({
        ...
        name: studentName, className, topics: studentTopics, avgMastery: avg,
        competencyAvg: { 
  ...
  const handleAIAnalyze = async () => {
    ...
    setAiInsight(null);
    try {
      const insight = await GeminiService.analyzeClassStrategy(students);
        ...
        <div className="absolute inset-0 z-[100] bg-background-dark flex flex-col animate-fade-in">
          ...
          <div className="flex-1 relative">
            <BubbleCanvas 
              ...
              preferences={{ theme: 'CRYPTO', showBreathing: true, showDrifting: true, showShimmering: true, fontSize: 16, intensity: 1, transparency: 0.8, brightness: 1, bubbleScale: 1.1 }} 
              onBubbleClick={() => {}} 
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {activeTab === 'CCTV' && (
            <div className="flex bg-white/5 p-1 rounded-2xl mr-4 border border-white/10">
              {GRID_MODES.map(m => (
                <button 
                  key={m.id} onClick={() => setGridMode(m)}
          ...
          <button 
            onClick={handleAIAnalyze} disabled={isGenerating || students.length === 0}
              ...
              {activeTab === 'CCTV' && (
                <div className={`grid gap-4 ${gridMode.cols} auto-rows-fr`}>
                  {students.slice(0, gridMode.rows).map((s, idx) => (
                    <div 
                      key={s.id} onClick={() => setFocusedStudentId(s.id)}
                      className={`relative group bg-white/[0.02] rounded-3xl border transition-all cursor-pointer flex flex-col p-4 ${s.status === 'CRITICAL' ? 'border-danger-glow/40 ring-4 ring-danger-glow/5' : 'border-white/5 hover:border-primary/50'}`}
                    >
